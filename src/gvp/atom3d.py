...
class PPIDataset(IterableDataset):
    ...
    :param db5: if true, then perform processing for the db5 dataset
        for now, this means taking all positive samples from the unbound structures
    '''
    def __init__(self, lmdb_dataset, db5=False):
        ...
        with torch.no_grad():
            for idx in indices:
                ...
                for i, (ensemble_name, target_df) in enumerate(pairs.groupby(['ensemble'])):
                    ...
                    if self.db5: 
                        # take the unbound structures
                        sub_names, (_, _, df1, df2) = get_subunits(target_df)
                        num_pos = positives_shape[0]
                    else:
                        # take the bound structures
                        sub_names, (df1, df2, _, _) = get_subunits(target_df)
                        num_pos = 10
                        
                    negatives = get_negatives(positives, df1, df2)
                    negatives['label'] = 0
                    labels = self._create_labels(positives, negatives, num_pos=num_pos, neg_pos_ratio=1)

                    for index, row in labels.iterrows():
                        label = float(row['label'])
                        chain_res1 = row[['chain0', 'residue0']].values
                        chain_res2 = row[['chain1', 'residue1']].values
                        graph1 = self._df_to_graph(df1, chain_res1, label)
                        graph2 = self._df_to_graph(df2, chain_res2, label)
